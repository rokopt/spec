...
Theorem plus_n_Sm : forall n m : nat,
  S (n + m) = n + (S m).
Proof.
  intros n m. induction n as [| n' IHn']. (** IHn': S(n'+m)= n'+(S m) **)
  - simpl. reflexivity.
...
Theorem plus_assoc : forall n m p : nat,
  n + (m + p) = (n + m) + p.
Proof.
  intros n m p. induction n as [|n' IHn']. (** IHn' :n'+ (m+p) = (n'+m)+p**) 
  - simpl. reflexivity.
  - simpl. rewrite -> IHn'. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros n. induction n as [|n' IHn'].  (** IHn': evenb (S n') = negb (evenb n') **)
  - simpl. reflexivity.
...
Proof.
  ...
  { rewrite -> plus_assoc. assert (H1: n+m=m+n). 
  { rewrite -> plus_comm. reflexivity. } rewrite -> H1. reflexivity. }
