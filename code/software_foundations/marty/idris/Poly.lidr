...
\idr{x}?
  ...
  - \idr{D (B A 5)} : well-typed: B A 5 returns Mumble, D takes in Mumble and returns Grumble x
  - \idr{D (B A 5) {x=Mumble}} yes
  - \idr{D (B A 5) {x=Bool}} yes
  - \idr{E True {x=Bool}}
...
> app_nil_r [] = Refl
> app_nil_r (x :: xs) = let inductiveHypothesis = app_nil_r xs in
>    rewrite app_nil_r xs in Refl
...
> rev_app_distr l1 [] = rewrite app_nil_r l1 in Refl
> rev_app_distr l1 (y :: xs) = ?rev_app_distr_rhs_2

> cons_hd_app : (y : x) -> (ys : List x) -> y :: ys = app [y] ys
> cons_hd_app y [] = Refl
> cons_hd_app y (x :: xs) = Refl
...
> split [] = PPair [] []
> split (z :: xs) =
>   let splitXS = split xs in PPair (fst z :: fst splitXS ) (snd z :: Poly.snd splitXS)
...
> filter_even_gt7 l = filter (lt 7) (filter (Numbers.evenb) l)
...
> partition f xs = PPair (filter f xs) (filter (not . f) xs)
...
>   fold_length_correct (y :: xs) = let inductiveHypothesis = fold_length_correct xs in rewrite inductiveHypothesis in Refl
...
>   fold_map f l = fold (\x,ly => f x :: ly) l []

Write down a theorem \idr{fold_map_correct} in Idris stating that \idr{fold_map}
is correct, and prove it.

>   fold_map_correct : (f : x -> y) -> (l : List x) -> Poly.map f l = Poly.Exercises.fold_map f l
>   fold_map_correct f [] = Refl
>   fold_map_correct f (y :: xs) = let inductiveHypothesis = fold_map_correct f xs in rewrite inductiveHypothesis in Refl
...
>   curry_uncurry f p = ?rhscurry_uncurry
