...
> or_commut (Left l) = Right l
> or_commut (Right r) = Left r
...
> not_implies_our_not notp q p = q
...
>   mult_0 : (n * m = Prelude.Nat.Z) <-> ((n = Prelude.Nat.Z) `Either` (m = Prelude.Nat.Z))
>   mult_0 {n} {m} = (to n m, or_example n m)
>   where
>     to : (n, m : Nat) -> (n * m = Prelude.Nat.Z) -> (n = 0) `Either` (m = 0)
>     to Prelude.Nat.Z _ Refl = Left Refl
>     to (S _) Prelude.Nat.Z _ = Right Refl
...
>   mult_0_3 : (n * m * p = Prelude.Nat.Z) <->
>              ((n = Prelude.Nat.Z) `Either` ((m = Prelude.Nat.Z) `Either` (p = Prelude.Nat.Z)))
>   mult_0_3 = (to, fro)
>   where
>     to : (n * m * p = Prelude.Nat.Z) -> ((n = Prelude.Nat.Z) `Either` ((m = Prelude.Nat.Z) `Either` (p = Prelude.Nat.Z)))
>     to {n} {m} {p} prf = let
...
>            Right prf => Right prf
>     fro : ((n = Prelude.Nat.Z) `Either` ((m = Prelude.Nat.Z) `Either` (p = Prelude.Nat.Z))) -> (n * m * p = Prelude.Nat.Z)
>     fro (Left Refl) = Refl
