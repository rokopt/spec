...
> snd_fst_is_swap p = case p of (n,m) => Refl
...
> fst_swap_is_snd p = case p of (n,m) => Refl
...
> nonzeros [] =[]
> nonzeros (Z :: x) = nonzeros x
> nonzeros (k :: x) = k :: nonzeros x
...
> oddmembers [] = []
> oddmembers (Z :: x) = oddmembers x
> oddmembers (k :: x) = if oddb k then k :: oddmembers x else oddmembers x
...
> countoddmembers [] = Z
> countoddmembers (k :: x) = if oddb k then S Z + countoddmembers x else countoddmembers x
...
> alternate [] (k :: x) = k :: x
> alternate (k :: x) [] = k :: x
> alternate (k :: x) (j :: y) = k :: j :: alternate x y
...
> count v [] = Z
> count v (k :: x) = if v == k then (S Z) + count v x else count v x
...
> member v [] =False
> member v (k :: x) = if v == k then True else member v x
...
> remove_one v [] = []
> remove_one v (k :: x) = if v == k then x else k :: remove_one v x
...
> remove_all v [] = []
> remove_all v (k :: x) = if v == k then remove_all v x else k :: remove_all v x
...
> subset [] s2 = True
> subset (k :: x) [] = False
> subset (k :: x) s2 = if (count k s2) < (count k (k :: x)) then False else subset x s2
...
> app_nil_r [] = Refl
> app_nil_r (n :: l1') = let inductiveHypothesis = app_nil_r l1' in
>     rewrite inductiveHypothesis in Refl

> rev_app_distr : (l1, l2 : NatList) -> rev (l1 ++ l2) = (rev l2) ++ (rev l1)
> rev_app_distr [] [] = Refl
> rev_app_distr [] l2 = rewrite app_nil_r (rev l2) in Refl
> rev_app_distr (k :: x) l2 =?rhsrev_app_distr
> --  let inductiveHypothesis = rev_app_distr x l2 in rewrite app_assoc l2 x [k] in rewrite inductiveHypothesis in Refl


> rev_involutive : (l : NatList) -> rev (rev l) = l
> rev_involutive [] = Refl
> rev_involutive (k :: x) =
>   let inductiveHypo = rev_involutive x in
>       rewrite rev_app_distr (rev x) [k] in rewrite inductiveHypo in Refl
...
> beq_NatList_refl (k :: x) = ?beq
>  --let inductiveHypothesis = beq_NatList_refl x in
> --  rewrite inductiveHypothesis in Refl
...
> remove_decreases_count (Z :: x) = rewrite ble_n_Sn (count 0 x) in Refl
> remove_decreases_count ((S k) :: x) =
>   let inductiveHypothesis = remove_decreases_count x in
>     rewrite inductiveHypothesis in Refl
...
> rev_injective [] l2 prf = ?rhsrev_involutive
> rev_injective (k :: x) l2 prf = ?rev_injective_rhs_2
...
> option_elim_hd (k :: x) default = Refl
...
> beq_id_refl (MkId (S k)) = let inductiveHypothesis = beq_id_refl (MkId k) in rewrite inductiveHypothesis in Refl
...
>   update_eq d x v = ?update_eq
> -- rewrite beq_id_refl x in Refl
>   --update_eq d (MkId Z) v = Refl
> -- rewrite beq_id_refl (MkId (S k)) in Refl
> -- seems I need to proof `if p then x else y` simplifies to x.
