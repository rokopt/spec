...
-- generates, for each type generated by `interpretSubst0Alg`, a type of
-- constraints on those types.
public export
data Subst0ConstraintF : {a : Type} -> Subst0TypeAlg a -> Type where

-- This dependent algebra combines `interpretSubst0Alg` and `XXX` to create
-- a functor in an arrow category.  (The arrow category is a sub-category of
-- the category of endofunctors on the substitution-0 category.  Consequently,
-- it has an initial object in the category of the F-algebras of the functor
-- generated by this dependent algebra, and we will use that initial algebra to
-- define, within the category of all initial algebras of endofunctors of
-- the substitution-0 category, the the object which represents the type of
-- all types of the _refined_ substitution-0 category.)
-- XXX
